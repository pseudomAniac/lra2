		...
		while (links.length>0) {
			...
			queryData(GAlink,(data)=>{
				if (data.rows) {
					...
					findOneAndUpdate({"nodeID": nodeID, "domain": country},{$set:{"pageviews":gaPageviews}},(err,doc)=>{
						console.log("updated -", doc.value.nodeID);
					});
				} else {
					console.log('+GA Data Missing -',nodeID);
				}
			});
		}
	}
