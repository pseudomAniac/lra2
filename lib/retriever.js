...
function cleaner (doc2update, callback) {
	...
	switch (doc2update.domain) {
		case "png":
			doc2update.pubdate = moment(new Date(tx[2]+", "+tx[1]+", "+tx[0])).subtract(10, "hours").unix();
			break;
		case "vanuatu":
			doc2update.pubdate = moment(new Date(tx[2]+", "+tx[1]+", "+tx[0])).subtract(11, "hours").unix();
			break;
		case "nauru":
			doc2update.pubdate = moment(new Date(tx[2]+", "+tx[1]+", "+tx[0])).subtract(12, "hours").unix();
			break;
		case "tonga":
			doc2update.pubdate = moment(new Date(tx[2]+", "+tx[1]+", "+tx[0])).subtract(13, "hours").unix();
			break;
		case "samoa":
			doc2update.pubdate = moment(new Date(tx[2]+", "+tx[1]+", "+tx[0])).subtract(14, "hours").unix();
			break;
		default:
			break;
	}
