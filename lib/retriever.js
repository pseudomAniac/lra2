...
exports.automateDataRetrieval = (country,pagesToScan,startScanAt)=>{
  ...
  console.log(country,pagesToScan,startScanAt);
    		...
    		db.articlescollection.findOne({"link":link, "domain":domain}, function(err, doc) {
    			if (!err) {
    				if (!doc) { // executes if doc is not found
    					...
    					db.articlescollection.findOneAndUpdate({"nodeID": doc.nodeID, "domain": doc.domain},(doc),{upsert: true});
    				}
    			}
