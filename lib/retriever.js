    		...
    		articlescollection.findOne({"link":link, "domain":domain}, function(err, doc) {
    			if (!err) {
    				if (!doc) { // executes if doc is not found
    					...
    					articlescollection.findOneAndUpdate({"nodeID": doc.nodeID, "domain": doc.domain},{$set:{"views":doc.views,"pubdate":doc.pubdate}});
    				}
    			}
...
function cleaner (doc2update, callback) {
	...
	doc2update.pubdate = moment(tx,"HH:mm, MMMM DD, YYYY").unix();
	console.log(moment.unix(doc2update.pubdate).subtract(10,"hours").format('HH:mm - MM/DD/YY'))
