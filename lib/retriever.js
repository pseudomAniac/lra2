...
var updateArticleInDB = Q.nbind(articlescollection.findOneAndUpdate.bind(articlescollection));
    						...
    						if (!err) {
    							cleaner(data, function(cleanedData) {
				    				if (!doc) { // executes if doc is not found
			    						...
			    						// ganal.getPageviews(cleanedData.link, (data)=>{
			    						// 	console.log(data)
			    						// })
	    								addArticle2DB(cleanedData);
				    				} else {
				    					articlescollection.findOneAndUpdate({"nodeID": doc.nodeID, "domain": doc.domain},{$set:{"views":cleanedData.views,"pubdate":cleanedData.pubdate}},(err,doc)=>{
				    						console.log("updated -", doc.value.nodeID, doc.value.views);
				    					});
				    				}
    							});
    						}
